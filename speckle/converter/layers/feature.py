    ...
    try:
        ...
        if selectedLayer.crs() != projectCRS:
            reprojectedPt = transform.transform(
                rasterOriginPoint, selectedLayer.crs(), projectCRS
            )
        ...
        logger.logToUser(
            "Error converting point geometry: " + str(error), Qgis.Critical
        )
        ...
        for h in range(rasterDimensions[0]):  # item in a row, X
            ...
            if selectedLayer.crs() != projectCRS:
                pt1 = transform.transform(
                    src=pt1, crsSrc=selectedLayer.crs(), crsDest=projectCRS
                )
                pt2 = transform.transform(
                    src=pt2, crsSrc=selectedLayer.crs(), crsDest=projectCRS
                )
                pt3 = transform.transform(
                    src=pt3, crsSrc=selectedLayer.crs(), crsDest=projectCRS
                )
                pt4 = transform.transform(
                    src=pt4, crsSrc=selectedLayer.crs(), crsDest=projectCRS
                )
            vertices.extend(
                [
                    pt1.x(),
            ...
            if rendererType == "multibandcolor":
                ...
                for k in range(rasterBandCount):
                    ...
                    colorVal = int(
                        (rasterBandVals[k][int(count / 4)] - rasterBandMinVal[k])
                        / valRange
                        * 255
                    )
                    if k + 1 == redBand:
                ...
                if rendererType == "contour":
                    try:
                        bandIndex = selectedLayer.renderer().inputBand() - 1
                    except:
                        try:
                            bandIndex = selectedLayer.renderer().band() - 1
                        except:
...
def cadFeatureToNative(feature: Base, attrs: QgsFields, layerName: str):
    ...
    for att in attrsToRemove:
        try:
            ...
            pass
    if "_Speckle_ID" not in dynamicProps:
    ...
    for name in dynamicProps:
        ...
        if name not in attrsToRemove:
            if name == "_Speckle_ID":
                variant = getVariantFromValue(feature["id"])
            else:
    ...
    for f in fields.toList():
        ...
        if f.type() == QVariant.LongLong:  # 4
            if isinstance(value, int):
                feat[fName] = int(value)
            else:
                logger.logToUser(
                    f"Value of the attribute '{fName}' of the layer '{layerName}' might be skipped due to type discrepancies",
                    Qgis.Warning,
                )
                try:
                    feat[fName] = int(value)  # 7524305913 is too big for Int
                except:
                    feat[fName] = None
