...
def traverseObject(
    ...
    if check and check(base):
        res = callback(base, streamBranch, plugin.qgis_project) if callback else False
        if res:
            return
...
def callback(base: Base, streamBranch: str, project: QgsProject) -> bool:
    if isinstance(base, VectorLayer) or isinstance(base, Layer) or isinstance(base, RasterLayer):
        ...
        layerToNative(base, streamBranch, project)
        time.sleep(0.3)
        #if layer is not None:
        #    .logToUser("Layer created: " + layer.name(), Qgis.Info)
    else:
        loopObj(base, "", streamBranch, project)    
    return True

def loopObj(base: Base, baseName: str, streamBranch: str, project: QgsProject):
    memberNames = base.get_member_names()
    for name in memberNames:
        ...
        try: loopVal(base[name], baseName + "/" + name, streamBranch, project)
        except: pass

def loopVal(value: Any, name: str, streamBranch: str, project: QgsProject): # "name" is the parent object/property/layer name
    ...
    if isinstance(value, Base): 
        try: # loop through objects with Speckletype prop, but don't go through parts of Speckle Geometry object
            if not value.speckle_type.startswith("Objects.Geometry."): 
                loopObj(value, name, streamBranch, project)
        except: 
            loopObj(value, name, streamBranch, project)
        ...
        for i, item in enumerate(value):
            loopVal(item, name, streamBranch, project)
            ...
            if item.speckle_type and item.speckle_type.startswith("IFC"): 
                ...
                pt, pl = cadLayerToNative(value, name, streamBranch, project)
                time.sleep(0.3)
