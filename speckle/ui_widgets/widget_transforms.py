...
FORM_CLASS, _ = uic.loadUiType(
    os.path.join(os.path.join(os.path.dirname(specklepy_qt_ui.qt_ui.__file__), "ui", "transforms.ui") )
)
        ...
        if len(self.layerDropdown.currentText())>1 and len(self.transformDropdown.currentText())>1:
            ...
            if exists == 0:
                ...
                if layer is not None:
                    ...
                    try: metrics.track("Connector Action", self.dataStorage.active_account, {"name": "Transformation on Send Add", "Transformation": listItem.split("  ->  ")[1], "connector_version": str(self.dataStorage.plugin_version)})
    ...
    def onRemoveTransform(self):
        ...
        if self.transformationsList.currentItem() is not None:
            ...
            if listItem in self.dataStorage.savedTransforms: 
                ...
                try: metrics.track("Connector Action", self.dataStorage.active_account, {"name": "Transformation on Send Remove", "Transformation": listItem.split("  ->  ")[1], "connector_version": str(self.dataStorage.plugin_version)})
                except Exception as e: logToUser(e, level = 2, func = inspect.stack()[0][3] )
